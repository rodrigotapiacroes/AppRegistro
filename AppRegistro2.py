...
""" En este script se trata automatizar la generacion de los graficos segun las combinaciones posibles entre las variables. Este script evita la actualizacion del codigo en Github. De modo que para cada grafico se puede generar ese mismo tipo de grafico segun la combinacion de variables seleccionables en la propia pagina web.

Limitaciones. El codigo esta basado exclusivamente en la oferta de datos codificada de BigQuery. Aunque, se puede establecer para un tipo de grafico distintas combinaciones, esta depende de la consulta inicial. 
...
try: 
    ...
    if df3.empty:
	st.warnings("No se encontraron datos validos. Revisa la tabla de BigQuery")

    else:
        st.sibheader("Ganancia Total por Fecha Agrupado por Producto")
	...
	#### Obtencion de la lista de los productos
        ...
        productos_seleccionados = st.multiselect(
             " 🍞 Seleccion que productos quieres comprar:",
             options = lista_productos,
             default=lista_productos[:3]
        )


        df_filtrado = df3[df3['Producto'].isin(productos_seleccionados)]

        if not df_filetrado.empty:
